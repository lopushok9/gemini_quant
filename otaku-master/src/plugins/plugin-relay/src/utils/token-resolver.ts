...
const HARDCODED_TOKEN_ADDRESSES: Record<string, Record<string, string>> = {
  ...
  arbitrum: {
    ...
    "usdc.e": "0xff970a61a04b1ca14834a43f5de4533ebddb5cc8", // Bridged USDC (legacy)
    usdce: "0xff970a61a04b1ca14834a43f5de4533ebddb5cc8", // Alias without dot
...
export async function getTokenMetadata(
  ...
  try {
    ...
    logger.info(
      `Successfully fetched token metadata: ${metadata.symbol} (${metadata.name}) - ${decimals} decimals`,
    );
    return metadata;
  } catch (error) {
...
export async function resolveTokenSymbol(
  ...
  if (hardcodedAddress) {
    logger.info(
      `Using hardcoded address for ${symbol} on ${network}: ${hardcodedAddress}`,
    );
...
export async function resolveTokenToAddress(
  ...
  if (trimmedToken.toLowerCase() === "eth") {
    ...
    logger.warn(
      `No WETH mapping found for ${network}, falling back to zero address`,
    );
  ...
  if (trimmedToken.startsWith("0x") && trimmedToken.length === 42) {
    ...
    if (metadata?.address) {
      logger.info(
        `Validated address ${token} exists on CoinGecko: ${metadata.symbol} (${metadata.name})`,
      );
      return metadata.address as `0x${string}`;
    }
    logger.warn(
      `Address ${token} not found on CoinGecko for network ${network} - may be fake/invalid`,
    );
    return null;
  }
...
export function addHardcodedTokenAddress(
  ...
  HARDCODED_TOKEN_ADDRESSES[network][symbol.toLowerCase()] =
    address.toLowerCase();
  logger.info(
    `Added hardcoded token address: ${symbol} on ${network} -> ${address}`,
  );
}
