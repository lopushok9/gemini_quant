...
export const cdpCheckTxConfirmation: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!txHash) {
        const errorMsg =
          "Missing required parameter 'hash'. Please provide a transaction hash.";
      ...
      if (!alchemyKey) {
        const errorMsg =
          "ALCHEMY_API_KEY not configured. Cannot check transaction confirmation.";
        logger.error(`[CHECK_TX_CONFIRMATION] ${errorMsg}`);
        const errorResult: ActionResult = {
          text: ` ${errorMsg}`,
          success: false,
      ...
      logger.debug(
        `[CHECK_TX_CONFIRMATION] Creating public client for ${network} with RPC: ${rpcUrl}`,
      );
      ...
      logger.info(
        `[CHECK_TX_CONFIRMATION] Fetching transaction receipt for ${txHash} on ${network}`,
      );
      callback?.({
        text: ` Checking transaction confirmation on ${network}...`,
      });
      ...
      try {
        ...
        try {
          ...
          logger.debug(
            `[CHECK_TX_CONFIRMATION] Could not fetch transaction for gas price: ${error}`,
          );
        }
        ...
        logger.warn(
          `[CHECK_TX_CONFIRMATION] Transaction not found or not mined: ${errorMessage}`,
        );
    ...
    [
      {
        name: "{{user}}",
        content: {
          text: "check confirmation for transaction 0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef on base",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: " Checking transaction confirmation on base...",
          action: "CHECK_TX_CONFIRMATION",
        },
      },
    ],
    [
      {
        name: "{{user}}",
        content: {
          text: "is transaction 0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890 confirmed?",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: " Checking transaction confirmation...",
          action: "CHECK_TX_CONFIRMATION",
        },
      },
    ],
    [
      {
        name: "{{user}}",
        content: {
          text: "check status of tx 0x9876543210fedcba9876543210fedcba9876543210fedcba9876543210fedcba on ethereum",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: " Checking transaction confirmation on ethereum...",
          action: "CHECK_TX_CONFIRMATION",
        },
      },
    ],
